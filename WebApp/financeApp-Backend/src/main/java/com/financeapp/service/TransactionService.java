...
public class TransactionService {
    ...
    public Map<String, Object> getFinancialSummary(User user) {
        ...
        // Get Udhaari summary
        Map<String, Object> udhaariSummary = udhaariService.getUdhaariSummary(user);
        BigDecimal netUdhaari = new BigDecimal(udhaariSummary.get("netUdhaari").toString());

        // Calculate proper net worth according to finance rules
        // Net Worth = (Cash from transactions) + (Current Investment Value) - Liabilities + Udhaari
        // Cash Balance = Total Income - Total Expenses
        BigDecimal cashBalance = totalIncome.subtract(totalExpenses);
        BigDecimal netWorth = cashBalance.add(investmentValue).add(netUdhaari);
        ...
        summary.put("netUdhaari", netUdhaari);  // Net Udhaari amount
        summary.put("netWorth", netWorth);  // True net worth including investments and Udhaari
